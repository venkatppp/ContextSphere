...
impl RecommendationGenerator for ContextRecommendationGenerator {
    async fn generate(&self, workspace_id: Uuid) -> Result<Vec<Recommendation>, DatabaseError> {
        ...
        // Workspace-scoped Smart Resume: only the requested workspace's latest
        // session is relevant for a proactive suggestion. The previous global
        // `get_smart_resume_session` could return a session from a different
        // workspace, causing the ProactiveAction's workspace_id to diverge from
        // the ProactiveNotification's workspace_id and triggering a
        // WorkspaceMismatch at execution (the screenshot bug).
        ...
        {
            Some(s) => s,
            None => return Ok(recommendations),
        };
